...
| 层级 | 默认位置 | 责任 | 变化方式 |
| --- | --- | --- | --- |
| Raw Sources | `${XDG_DATA_HOME:-$HOME/.local/share}/qatlasd/raw/`，或显式覆盖 `QATLAS_RAW_DIR` | PDF、Markdown、解析结果、运行资产等可追溯证据 | 尽量追加，不随意改写 |
...
每个页面带 YAML frontmatter（`id`、`title`、`type`、`status` 等必填），文件名遵循 `algo-*` / `prim-*` / `person-*` / `paper-arxiv-*` / `comp-*` 等前缀规范。完整模板、frontmatter schema、lint 错误码和同步规则见 [docs/reference/wiki-schema.md](docs/reference/wiki-schema.md)。
...
QuantumAtlas 偏向「研究基础设施」而不是静态资料库。**配置分两边**：

- **Client (Python `qatlas` CLI)**：自 v0.17.0 起 **YAML-only**（路径按平台：Linux `~/.config/qatlas/config.yaml`、macOS `~/Library/Application Support/qatlas/config.yaml`、Windows `%APPDATA%\qatlas\config.yaml`，由 [`platformdirs`](https://platformdirs.readthedocs.io/) 解析）。**首次跑任何 `qatlas <cmd>` 自动创建模板**，用 `qatlas config set` 维护；不再读 CLI flag / OS env / `QATLAS_DOTENV`。
- **Server (Go `qatlasd`)**：三入口 CLI flag > OS env > `.env` 文件 > default（由 godotenv 加载，systemd unit 通常 `Environment=QATLAS_DOTENV=/etc/quantum-atlas/.env`）；20 个 serve 子命令 flag 每个都标 `[env: QATLAS_FOO=]`（easytier 风格）。

server 项目自有字段统一用 `QATLAS_` 前缀；多人协作时通常什么都不用写——所有存储路径都有默认：`QATLAS_WIKI_DIR` 默认是兄弟 checkout `../QuantumAtlas-Wiki`；`QATLAS_RAW_DIR` / `QATLAS_DATA_DIR` / `QATLAS_PB_DATA_DIR` 默认走 `${XDG_DATA_HOME:-$HOME/.local/share}/qatlasd/{raw,data,pb_data}`，跟 git checkout 完全分离。需要存到挂载盘或 `/var/lib/` 时再显式覆盖。

内容贡献分两条并列路径：

- **Raw 资产**走 `QATLAS_RAW_DIR`（默认 `$XDG_DATA_HOME/qatlasd/raw`），按 YYMM 分片存储（如 `<raw_dir>/pdf/9508/9508027v1.pdf`、`<raw_dir>/pdf/2501/2501.00010v1.pdf`）。三种方式都会落到同一布局：
...
- [docs/reference/upload-api.md](docs/reference/upload-api.md): `qatlas upload …` / `POST /api/papers/.../upload-pdf` 完整 API 参考（sha256 dedup、idempotent retry、in-transit guard、conflict 处理）。
...
提交说明请使用 Conventional Commits，例如 `feat:`、`fix:`、`docs:`、`refactor:`、`test:`、`chore:`。版本发布由 Commitizen 统一维护，细节见 [docs/contributing.md](docs/contributing.md)。
