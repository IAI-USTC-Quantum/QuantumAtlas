...
type WorkerSnapshot = {
  id: number; source: string | null; terminationCalls: number; replies: number
  postedAt: number | null; terminatedAt: number | null; errors: string[]
}

// Keep real, same-origin native Workers: this controls only the message boundary,
// not the network/CSP or the native terminate() implementation. Saving the
// installed callback lets a test also simulate a reply already queued at cleanup.
export async function controlMarkdownWorkers(page: Page) {
  await page.addInitScript(() => {
    ...
    class ControlledWorker extends Worker {
      ...
      errors: string[] = []
      queuedHandler: Worker['onmessage'] = null
      ...
      constructor(url: string | URL, options?: WorkerOptions) {
        ...
        this.addEventListener('error', (event) => {
          ...
          // The UI intentionally handles/prevents Worker errors. Record them
          // independently so a startup failure cannot masquerade as a timeout
          // or as rejection of a synthetic reply; do not suppress the event.
          console.error(`BROWSER_REGRESSION_WORKER: ${error}`)
        })
        this.addEventListener('messageerror', () => {
          this.errors.push('Unexpected native Worker message deserialization error')
        })
      }

      postMessage(message: unknown) {
        // The real Worker starts, but its parse job is withheld; replies are
        // injected separately. A timeout therefore exercises the real UI timer.
        this.source = typeof message === 'string' ? message : null
        this.postedAt = performance.now()
        this.queuedHandler = this.onmessage
      }

      terminate() {
        super.terminate()
        this.terminatedAt ??= performance.now()
        this.terminationCalls++
      }

      reply(data: unknown, alreadyQueued: boolean) {
        if (this.errors.length) throw new Error(`Native Markdown Worker failed: ${this.errors.join('; ')}`)
        if (!alreadyQueued && this.terminationCalls) throw new Error('Cannot inject a normal reply into a terminated Markdown Worker')
        const event = new MessageEvent('message', { data })
        this.replies++
        if (alreadyQueued) {
          if (!this.queuedHandler) throw new Error('No captured Markdown Worker message callback')
          this.queuedHandler.call(this, event)
        } else {
    ...
    Reflect.set(window, '__controlledMarkdownWorkers', {
      snapshots: () => workers.map(({ id, source, terminationCalls, replies, postedAt, terminatedAt, errors }) => ({
        id, source, terminationCalls, replies, postedAt, terminatedAt, errors,
      })),
      reply: (id: number, data: unknown, alreadyQueued: boolean) => {
        if (!workers[id]) throw new Error(`Unknown Markdown Worker ${id}`)
        workers[id].reply(data, alreadyQueued)
      },
    })
  })
  const snapshots = async (): Promise<WorkerSnapshot[]> => {
    const result: WorkerSnapshot[] = await page.evaluate(() => Reflect.get(window, '__controlledMarkdownWorkers').snapshots())
    expect(result.flatMap(({ errors }) => errors), 'Controlled Workers must have no native startup/runtime errors').toEqual([])
    return result
  }
  return {
    snapshots,
    reply: async (id: number, data: unknown, alreadyQueued = false) => {
      await page.evaluate(async ({ id, data, alreadyQueued }) => {
        Reflect.get(window, '__controlledMarkdownWorkers').reply(id, data, alreadyQueued)
        // Give React a paint after delivery; stale-reply assertions must not pass
        // merely because the update was still queued when we examined the DOM.
        await new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve())))
      }, { id, data, alreadyQueued })
      await snapshots()
    },
  }
}
