...
function PATPage() {
  ...
  // When the scope vocabulary loads (or the user opens the dialog
  // before any have been selected), default to "all scopes checked".
  // Reasoning: a PAT is a power-user thing and the user can always
  // uncheck. Forcing zero-by-default puts users one extra round of
  // clicking from a working token without making them any safer —
  // they were going to click the boxes anyway.
  useEffect(() => {
    if (!creating) return
    if (!scopes.data) return
    if (selectedScopes.size > 0) return
    setSelectedScopes(new Set(scopes.data.scopes.map((s) => s.name)))
