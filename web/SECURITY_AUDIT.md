...
> 后续按明确需求重新接入了 Markdown / LaTeX 渲染，现直接使用 react-markdown 与单版本
> KaTeX；marked、DOMPurify 和旧 Worker/JSON 渲染链均已移除。
> 当前的资源预算、原文模式、安全策略与性能实测见 [Markdown 与公式预览](MARKDOWN_PREVIEW.md)。
