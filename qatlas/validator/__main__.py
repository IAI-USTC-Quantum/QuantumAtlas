...
if TYPE_CHECKING:
    ...
    from qatlas.validator.test_framework import TestSuite
...
def load_circuit(file_path: str) -> "QuantumCircuit":
    """从裸 QuantumCircuit JSON 或 QuantumIR JSON 文件加载电路"""
    path = Path(file_path)
    ...
    circuit_data = data["circuit"] if "circuit" in data else data
    return QuantumCircuit.from_dict(circuit_data)
...
def load_test_suite(file_path: str) -> "TestSuite":
    ...
    from qatlas.validator.test_framework import TestSuite, TestCase
    import numpy as np
