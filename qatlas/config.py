...
class ServerConfig(BaseSettings):
    ...
    # Which first-party plugins are ENABLED — config-driven, not hardcoded. A list of
    # builtin plugin names (`lean`, `claim`); a plugin contributes CLI commands only
    # when listed here (and its available() env-check passes). Absent → the default
    # set (`lean` only; `claim`, the localhost claim-drafting WebUI, is opt-in — ADR
    # 0008 moved the maintained flow to qatlas-lean). Example config.yaml:
    #   plugins: [lean, claim]
