...
// agenticHarness mounts POST /api/search/agentic with the given remote
// provider (nil = search.remote disabled) over a real test PB app.
...
func newAgenticHarness(t testing.TB, remote *search.RemoteProvider) *agenticHarness {
	t.Helper()
	...
	err = app.OnServe().Trigger(se, func(e *core.ServeEvent) error {
		RegisterSearchAgentic(e, &config.Config{}, remote, usage.NewStore(nil), search.NewEngine(nil, nil), enforcer)
