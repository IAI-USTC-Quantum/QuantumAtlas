...
class WikiIngester:
    ...
    Usage:
        engine = WikiEngine()
        result = engine.ingester.ingest_paper("9508027")
    """
    ...
    def arxiv_fetcher(self):
        ...
        if self._arxiv_fetcher is None:
            ...
            pdf_dir = self.engine.raw_dir / "papers" / "pdf"
    ...
    def pdf_parser(self):
        ...
        if self._pdf_parser is None:
            from atlas.parser.pdf_parser import PDFParser
    ...
    def extractor(self):
        ...
        if self._extractor is None:
            from atlas.extractor.extractor import create_extractor
    ...
    def ingest_paper(
        ...
        try:
            ...
            if parse:
                ...
                markdown_path = self._parse_pdf(arxiv_id, metadata if fetch else None)
            ...
            self.engine.append_to_log(f"[INGEST] {arxiv_id}: Created {len(result.get('wiki_pages', []))} pages")
    ...
    def _fetch_paper(self, arxiv_id: str) -> tuple:
        ...
        pdf_path, metadata = self.arxiv_fetcher.fetch(arxiv_id)

        # Save metadata to JSON
        json_path = self.engine.raw_dir / "papers" / "json" / f"{arxiv_id}.json"
        json_path.parent.mkdir(parents=True, exist_ok=True)
        json_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")

        return pdf_path, metadata

    def _parse_pdf(self, arxiv_id: str, metadata: Optional[Dict] = None) -> Path:
        """Parse PDF to Markdown."""
        pdf_path = self.engine.raw_dir / "papers" / "pdf" / f"{arxiv_id}.pdf"

        if not pdf_path.exists():
            raise FileNotFoundError(f"PDF not found: {pdf_path}")

        markdown_path = self.engine.raw_dir / "papers" / "markdown" / f"{arxiv_id}.md"
    ...
    def _create_wiki_pages(
        ...
        if "fetch" in ingest_result["steps"]:
            ...
            if "extract" in ingest_result["steps"]:
                ...
                algo_id = f"algo-{extract_info['algorithm_id'].replace('_', '-').lower()}"
        ...
        if "extract" in ingest_result["steps"]:
            ...
            algo_id = f"algo-{algo_name.replace('_', '-').replace(' ', '-').lower()}"

            # Map primitive IDs to wiki format
            primitive_ids = [
                f"prim-{p.replace('primitive_', '').replace('_', '-')}"
                for p in extract_info.get("primitives", [])
            ]
            ...
            algo_page = PageTemplate.algorithm_entity(
                ...
                problem="Unknown",  # Would need to extract this
                description=f"Algorithm extracted from arXiv:{arxiv_id}",
