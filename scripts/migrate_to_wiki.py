...
def migrate_papers(
    ...
    """
    Migrate existing papers directory to raw/ and wiki/.
    ...
    for subdir in ["pdf", "markdown", "json"]:
        (raw_dir / "papers" / subdir).mkdir(parents=True, exist_ok=True)
    wiki_sources_dir.mkdir(parents=True, exist_ok=True)

    # Migrate PDFs
    for pdf in old_papers_dir.glob("*.pdf"):
        target = raw_dir / "papers" / "pdf" / pdf.name
        if dry_run:
    ...
    for md in old_papers_dir.glob("*.md"):
        target = raw_dir / "papers" / "markdown" / md.name
        if dry_run:
    ...
    for js in old_papers_dir.glob("*.json"):
        target = raw_dir / "papers" / "json" / js.name
        if dry_run:
        ...
        try:
            ...
            wiki_path = wiki_sources_dir / f"arxiv-{arxiv_id}.md"
            if dry_run:
                logger.info(f"  [DRY-RUN] Would create: {wiki_path}")
            else:
                page.save(wiki_path)
                logger.info(f"  Created wiki page: arxiv-{arxiv_id}.md")
            stats["wiki_pages"] += 1
...
def update_gitignore(project_root: Path, dry_run: bool = False) -> None:
    ...
    new_entries = [
        ...
        "raw/papers/pdf/*.pdf",
        "raw/papers/markdown/*.md",
        "raw/papers/json/*.json",
        "wiki/log.md",
    ]
