...
客户端由 [qatlas-cli 独立仓库](https://github.com/IAI-USTC-Quantum/qatlas-cli)维护和发版，安装包为 [PyPI `qatlas-cli`](https://pypi.org/project/qatlas-cli/)。安装本仓不会提供 `qatlas` 命令；已装过旧包 `quantum-atlas` 的用户请先完成[迁移](#从旧包-quantum-atlas-迁移)。
...
<a id="migrate-quantum-atlas"></a>

## 从旧包 quantum-atlas 迁移
