...
跨组件的稳定约定：环境变量、arXiv ID 格式。需要查具体字段 / flag / 格式时来这里。

```{admonition} CLI 与 API 参考在组件分节下
:class: tip
...
- **[环境变量](env-vars.md)**
  ...
  全部 `QATLAS_*` + 第三方 SDK 标准名（`MINERU_*` / `GITHUB_*` 等），分 client / server / 共享。

- **[arXiv ID 格式](arxiv-ids.md)**
