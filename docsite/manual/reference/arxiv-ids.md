...
~~~{admonition} 用了 `arxiv.org/abs/...` 完整 URL
:class: caution
...
~~~{admonition} 老式带版本是合法的
:class: tip

`quant-ph/9508027v1`（含斜杠 + 版本）—— 是合法的，正常工作。
~~~

~~~{admonition} 老式 bare 形态（`9508027v1`，无 category）当前仍接受但**有歧义**
:class: warning
...
~~~{admonition} DOI 入口同一个端点
:class: note

`GET /api/papers/{id_or_doi}/markdown` 自动 detect DOI（`^10\.\d{4,9}/`），
经 OpenAlex 反查成 arxiv_id 后走同一套 handler。详见 [REST API · DOI 寻址](../server/rest-api.md#doi-入口)。
~~~
