...
~~~{admonition} 旧 `qatlas ingest` 命令已移除
:class: note
...
```{admonition} MinerU 是当前唯一受支持的解析器
:class: note
...
- 已跑 `qatlas auth login -s <host>` 拿到 PAT（写进 hosts.yml；静默抓取
  ...
  [怎么拿 PAT](manage-credentials.md#创建-pat)）
...
```{admonition} 404 Paper not found on arXiv
:class: warning

arxiv_id 拼错（注意旧式带分类前缀 `quant-ph/...`，新式没前缀 `2501.00010`）。
```

```{admonition} 503 MinerU not configured on server
:class: warning
