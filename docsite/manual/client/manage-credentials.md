...
```{admonition} 401 Unauthorized 但我配了 PAT
:class: warning

检查 token 是否过期：`/pat` 页面看 expires_at。如果是 session token (JWT)，14 天到期需要重登。
```

```{admonition} 403 insufficient scope: this token lacks papers:write
:class: warning

PAT 创建时没勾 `papers:write`。要么 revoke + 重建带 scope 的，要么用 session token。
```

```{admonition} 403 this endpoint requires a browser session token
:class: warning
...
```{code-block} yaml
:caption: ~/.config/qatlas/hosts.yml
