...
```{admonition} 旧包 quantum-atlas 退役
:class: warning

`quantum-atlas 0.21.0` 是仅含元数据的最终迁移版，不含 `qatlas` 模块、parser 库或 console entry，没有运行时依赖，也不会自动安装 `qatlas-cli`。安装 QuantumAtlas 主仓同样不会提供 `qatlas`。旧用户请按[迁移步骤](../getting-started.md#从旧包-quantum-atlas-迁移)先卸载旧包，再用原安装器安装 `qatlas-cli`；不要删除 `~/.config/qatlas`。
```
...
~~~{admonition} v0.17.0 删除了 `--base-url` / `--token` / `--insecure` flag
:class: note
...
```{admonition} 想换 config 文件位置？用平台标准 env
:class: note
...
~~~{admonition} `qatlas config set` 会抹掉手写注释
:class: warning
