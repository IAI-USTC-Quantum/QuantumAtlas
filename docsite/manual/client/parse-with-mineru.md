...
## 贡献者路径：`qatlas contrib mineru`

为什么走贡献者本地：

1. **配额归属**——贡献者用自己的 MinerU token 跑，不吃 server 共享配额
2. **批量贡献**——队列 / `--watch` daemon 模式可一次攒很多篇，主动暖缓存
3. **自托管 MinerU**——把 `mineru_api_base_url` 指向自己的 MinerU 实例，
   仍走 `qatlas contrib mineru` 同一条路
...
PDF 必须**已经在 server 上**（通过 `qatlas ingest` 或 `qatlas contrib pdf` 推上去）。
...
```bash
qatlas contrib mineru                  # 默认 batch_size=50（MinerU 单批硬上限）
qatlas contrib mineru --batch-size 20  # 想缩小批量
```
...
```bash
# 后台跑 + 把 stderr 重定向到日志
nohup qatlas contrib mineru --watch --batch-size 50 > qatlas-mineru.log 2>&1 &
```
...
```bash
tmux new -s mineru
qatlas contrib mineru --watch --batch-size 50
...
```bash
nohup qatlas contrib mineru --watch > qatlas-mineru.log 2>&1 &
disown
...
[Service]
Type=simple
ExecStart=%h/.local/bin/qatlas contrib mineru --watch --batch-size 50
Restart=on-failure
RestartSec=30
...
bash mode="async" shellId="mineru-watch"
  command: qatlas contrib mineru --watch --batch-size 50 2>&1 | tee /tmp/qatlas-mineru.log

# 过一阵子拉新 stderr 输出：
read_bash shellId="mineru-watch"

# 优雅停：
bash: pgrep -af "qatlas contrib mineru" | head -2
...
加 [通用 client flags](manage-credentials.md#通用-client-flag)。

## 错误分类（v0.15.0+）

`qatlas contrib mineru` 把 MinerU 返回的 17 种 fatal code + 9 种 retryable code + 2 种 daily-limit code 按行为分三类：
...
```{admonition} v0.15.0：batch + daily-limit
:class: note
...
`qatlas contrib mineru` 自动处理整个生命周期（含异常时的释放），手工调 API 时务必保证 release。

## 常见问题

````{admonition} MINERU_API_TOKENS must be set
:class: warning
...
```{admonition} skip (HTTP 409): paper already has markdown
:class: warning

Server 已有 markdown。要么跳过，要么 `--overwrite`。
```

```{admonition} skip (HTTP 409): paper already claimed by other client
:class: warning

别人正在跑。等 30 分钟租约过期，或换一篇。
```

```{admonition} MinerU task did not finish within MINERU_TIMEOUT=1800s
:class: warning

大论文（>50 页带很多图表）需要更长时间。`export MINERU_TIMEOUT=3600` 或单篇模式跑。
```

```{admonition} [daily-limit] MinerU quota exhausted; exiting 75 (EX_TEMPFAIL)
:class: warning

今日 5000 篇免费额度用完。一次性运行退出 75；CI 把 75 视为 transient，下次任务自然重试。要立即继续就改用 daemon 模式（`--watch`），它会自动 sleep 到次日 00:01。
```

```{admonition} [fatal] MinerU rejected batch submission: code -60006 (文件页数超过限制（最多 200 页）)
:class: warning

MinerU 单文件上限 200 页。本 paper 自动 skip + release lease；client 不重试。要解析需先手工拆分 PDF（**`qatlas contrib mineru` 不实现客户端 split**——拆完后再 ingest）。
```

```{admonition} [fatal] MinerU rejected batch submission: code A0202 (Token 错误)
:class: warning

`mineru_api_token` 错或带 `Bearer ` 前缀。换 token 即可；client 不重试。
```

```{admonition} [fatal] MinerU rejected batch submission: code A0211 (Token 过期)
:class: warning

去 MinerU 重新拿 token，`qatlas config set mineru_api_token` 后重跑。
```

```{admonition} Markdown upload for X failed: HTTP 400 ... expected_sha256 mismatch
:class: warning
