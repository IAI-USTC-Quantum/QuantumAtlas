...
```{dropdown} 为什么现在用 PostgreSQL，而不是把索引放进 RustFS
...
完整原理见 [`architecture.md` § PostgreSQL catalog](architecture.md#论文元数据索引postgresql-catalog)。
```

```{dropdown} 同一个 PostgreSQL 里还有 OpenAlex 语料（corpus）
...
```{dropdown} 为什么是 arxiv ID 而不是 content-addressed（sha256 寻址）
...
```{dropdown} 为什么从 qatlas-raw 拆出 5 桶（v0.7.0 设计决策）
