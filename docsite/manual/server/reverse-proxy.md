...
详见 [RustFS 部署](rustfs.md#dual-endpoint-mode)。
...
```{code-block} nginx
:caption: /etc/nginx/sites-available/quantum-atlas.conf
...
每条线路一台反代，对应一台 qatlasd。共享后端 (RustFS / PostgreSQL) 通过 EasyTier mesh 互通。各边缘 PocketBase 独立（用户/PAT 不跨节点）。
