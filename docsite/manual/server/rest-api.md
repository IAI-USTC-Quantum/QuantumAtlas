...
> 历史镜像说明：这份 spec 也嵌进了文档站：[API Explorer](api-explorer.md) 页可在浏览器里
> 展开浏览全部 endpoint 的参数 / schema（静态镜像，在线点测仍用上面的 `/swagger`）。

上段保留的是旧版文档镜像的说明。当前 Sphinx 站点不运行旧版镜像 hook，API Explorer
改为提供原生 OpenAPI spec 下载与当前实例 `/swagger/` 的普通链接；本页的静态表格
仍可直接阅读。Swagger UI 本身不是只读界面，真实 API 调用须由用户主动选择操作，
并通过服务端鉴权。
...
> 警告：注解里的 path / 参数 / 响应是**手写声明**，不是从真实闭包反射出来的——swaggo 在任何
...
[Robust Downloader · 管理员补救流](downloader.md#管理员补救流失败闭环)。
...
|---|---|---|---|
| `cached` | 是 | 是 | 直接 GET 资源拿字节 |
| `queued` / `running` | 否 | 否 | 等 `Retry-After`，再 poll |
| `running` | 是 | 否 | 等 MinerU 收尾，之后 GET `/markdown` |
| `cooldown` | 是 | 否 | 到 `retry_after_iso` 之前不再 poll |
| `failed` `kind=fatal` | 否 | 否 | 永久失败，放弃 |
| `failed` `kind=retryable` | 否 / 是 | 否 | 到 `retry_after_iso` 后重试 |

<a id="doi-addressing"></a>
