...
- 带权威身份（DOI / arXiv ID）的命中进入 ``results``，并登记论文元数据，
  不自动下载（见 :doc:`concepts`；``created`` 仅表示新建元数据记录）；
...
普通、逐平台、Agentic 和自定义评分搜索都只返回结果并登记元数据，不会自动
提交下载任务。结果默认不勾选；勾选需要的论文后，点击「下载所选」，前端才
调用 ``POST /api/downloader/fetch``。主服务校验 ``papers:write`` 权限，
仅为本次明确提交的条目创建任务，worker 继续按原有方式领取和执行。
仅有标题、没有可用下载标识的条目不能提交下载。

.. note::

   此策略只影响之后的搜索。升级前已经排队或执行中的任务不会自动取消；
   如需清理旧队列，必须单独处理。
...
只对最终返回的身份可确认论文登记元数据，下载仍需勾选后明确提交。过滤与评分基于有限召回候选，
