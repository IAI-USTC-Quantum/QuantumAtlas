...
下载任务执行时可经历 ``pending``\（资产抓取中）→ ``ready``。
仅有元数据的论文也可使用注册表默认的 ``ready`` 状态，因此是否已有 PDF
应以资产记录为准，不能仅凭 ``status`` 判断。
...
系统会为它铸造 ``paper_id``、登记元数据，但不会自动抓取 PDF 或创建下载任务。
在搜索结果中勾选论文，再点击「下载所选」，主服务才为所选论文提交下载任务。

搜索响应中的 ``created: true`` 仅表示本次搜索新建了元数据记录，不代表已下载。
此策略适用于之后的搜索；升级前已排队的任务仍会继续执行，不会自动取消。
