...
**资产浏览（Asset Browser）**
    ...
    - 在线预览（PDF 内嵌 viewer / Markdown 与 LaTeX 公式，支持切换原文）
    - 代理流式下载
    - 生成预签名 S3 URL（浏览器直连，默认 1h 有效）
    - 复制 S3 object key
    - 批量 ZIP 下载（最多 20 篇）

    论文详情页的管理员资产面板使用相同预览器。公式支持 ``$...$`` / ``$$...$$``
    和 TeX 括号分隔符；这不是完整的 LaTeX 文档编译器。代码块保持原样，原始 HTML
    不执行，图片不自动加载。超过 200,000 字符、结构过于复杂或渲染失败时，请使用
    “原文”模式或下载文件；原始资产不会被修改。
