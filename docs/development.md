...
binary 装到 `~/.local/bin/quantumatlas`（无 sudo 写入），systemd unit 视部
...
实际由 `internal/routes/*.go` 与 `cmd/server/main.go` 注册。鉴权列规则：
...
```text
QuantumAtlas/
├── cmd/server/            Go binary entrypoint（生产 server）
