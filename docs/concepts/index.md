...
-   :material-layers-triple:{ .lg .middle } **[三层架构](architecture.md)**
...
-   :material-database-cog:{ .lg .middle } **[存储架构](storage-architecture.md)**
    ...
    对象存储 (RustFS/S3) + Metadata DB (SQLite) + Graph (Neo4j) 三层切分，以及对象寻址、sha256 dedup、bucket versioning、桶布局与对账等机制。
