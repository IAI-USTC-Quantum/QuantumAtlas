...
上面三种身份（session / PAT / 匿名）全都依赖 **PocketBase 在跑**：session JWT 要 PB 验签、PAT 要查 `pat_tokens` 表。当 pb_data 不可用——新 edge 还没人 OAuth 登过、SQLite 损坏需要灾难恢复、CI 想跑但不想绑某个具体人的身份——这三条全断。

**System PAT** 是兜底的第四种身份，**只通过环境变量**配置：
