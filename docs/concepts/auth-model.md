...
| **匿名** | 任何人 | — | 不带 `Authorization` 头 | 仅读公开端点 |
...
| Scope | 覆盖端点 |
|---|---|
| `papers:write` | `POST /api/papers/{id}/upload-pdf` `POST /api/papers/{id}/upload-markdown` `POST /api/papers/{id}/mineru-claim` `DELETE /api/papers/{id}/mineru-claim/{cid}` |
| `shares:read` | `GET /api/shares/` |
| `shares:write` | `POST /api/shares/`、`DELETE /api/shares/{token}`（隐式含 `shares:read`）|
| `graph:read` | `GET /api/graph/stats`、`GET /api/graph/schema`、`POST /api/graph/query` |
| `wiki:write` | `POST /api/wiki/sync/pull`（服务端 git fast-forward + 缓存刷新）|

**Scope 是编译时静态的**——加新 scope 需要改代码 + 重新部署。完整词表在 `internal/pat/scopes.go`。

!!! warning "PAT 默认空集"
    创建 PAT 时**至少要勾一个 scope**，否则该 PAT 调任何写口都是 403。读口（Wiki / pages / search / health 等）不受 scope 影响，匿名都能读。
...
- **`authGuard`** — 只要有效凭据就放行；公开读端点甚至不挂这层。
- **`scopeGuard(obj, act)`** — 在 `authGuard` 之上再检查 PAT 是否包含覆盖 `(obj, act)` 的 scope。**session 自动 bypass**（它有隐式 `*`）。所有写口都挂这层。
...
| 类别 | 鉴权 |
|---|---|
| Wiki 读 (`/api/wiki/sync/status`、`/api/pages`、`/api/search`、`/api/stats`、`/api/lint`) | **公开**|
| Graph 读 / 查询 (`/api/graph/*`) | `authGuard + graph:read` |
| Health / Meta (`/api/health`、`/api/server/info`、`/install-server.sh`) | **公开**|
...
设计原则：**Wiki 是公开仓库**，所以 Wiki 读接口全开；防御只针对写。

!!! note "Graph 是读口里的例外"
    Wiki 读口公开（数据本就在公开 git 仓库里），但 **Graph 读 / 查询例外**，要 `authGuard + graph:read`。两个原因：(1) 一致性——其余非公开仓库表面都走同一套 `authGuard/scopeGuard`，graph 曾是唯一无鉴权的口，收敛掉这个「为什么就它特殊」的隐患；(2) `/api/graph/query` 执行调用方提供的 Cypher。

    查询**只读**（驱动层 `ExecuteRead` 拒绝写），但**故意不加查询代价上限**：过了 `graph:read` 的调用方即「自己人」，同一个人本就能直连 Bolt 跑同样的重查询，应用层限制器挡不住、只增复杂度。病态查询（如无界笛卡尔积）能拖垮 Neo4j，**唯一缓解是撤销出问题的凭据**。这是明确接受的风险，不是待办——细节见 [REST API · graph/query](../reference/rest-api.md) 与 [Neo4j 部署](../deployment/neo4j.md)。
