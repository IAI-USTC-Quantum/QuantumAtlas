...
早期 wiki 区分 `entity`（算法/原语/人物）、`comparison`（对比页）、`source`（论文）等多种页面类型，结果是同一件事散落在好几种结构里——读者要先理解"页面类型体系"才能浏览，作者要纠结"这该建 entity 还是 comparison"。
...
QuantumAtlas 的生产部署横跨海外 + 国内两条线路。两边都是完整 qatlasd，共享底层的 RustFS + Neo4j（通过 EasyTier mesh）。
...
PAT scope 词表（`papers:write` / `shares:read` / `shares:write` / `graph:read` / `wiki:write`）默认**空集**。新建 PAT 不勾任何 scope = 这个 PAT 啥都干不了。
