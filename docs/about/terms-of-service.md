...
- <https://quantum-atlas.ai>（RackNerd 海外节点）
- <https://47.102.36.175>（阿里云国内节点，自签证书）

也可自行部署（见 [安装](../README.md#安装)）。
...
- ❌ **商业产品集成**（包括 SaaS / 付费 API 转售）—— 涉及 MinerU markdown 全文
  时尤其禁止（解析输出本身受原 PDF 版权约束，见
  [License & Attribution](license-and-attribution.md#我们怎么处理pdf--metadata--全文三类数据)）
- ❌ **大规模无视 rate limit 的爬取**（如果给服务造成负担，我们可能撤销 PAT）
- ❌ **用 share token 批量下载 PDF 后再分发**——share token 是给协作者**单次访问**
  用的；PDF 字节归 arxiv 与原作者
- ❌ **滥用 `/api/graph/query`** 跑病态 Cypher（如无界笛卡尔积）拖垮 Neo4j——
  detail 见 [鉴权模型](../concepts/auth-model.md#graph-查询同-scope-下危害最大的那一档)
- ❌ **拿用户数据训练公开 LLM 模型**——MinerU markdown 与 Wiki 内容均不构成
  "公开 corpus"
...
- 论文 **PDF 字节** 我们**不**持有也**不**分发——share / download 接口 `307 redirect`
  到 `https://arxiv.org/pdf/<id>.pdf`
- 论文 **MinerU 解析后的 markdown 全文** require login，仅研究 / 教育用
- **Wiki 内容** MIT，在 [QuantumAtlas-Wiki repo](https://github.com/IAI-USTC-Quantum/QuantumAtlas-Wiki) 自维护
...
- 浏览公开端点（health / server-info / install-server / share 短链 / SPA 外壳）
...
- 上传到 `qatlas-{pdf,md}` 桶的内容**全员可见**（同登录用户读得到，本服务不做
  per-user 隔离）——不要上传敏感 / 私密内容

## 终止

- 你**随时**可停止使用，撤销所有 PAT，删除你创建的 share token
- **删除账号**：提 [GitHub issue](https://github.com/IAI-USTC-Quantum/QuantumAtlas/issues)
  或邮件联系（[维护者](credits.md#维护者)）；我们删 PocketBase 用户记录 + 你创建的
  share / claim / 上传记录。OpenAlex 上的 metadata 不归我们管，要删请直接联系上游
...
（即 [MIT LICENSE](https://github.com/IAI-USTC-Quantum/QuantumAtlas/blob/main/LICENSE)
