...
让 server 直接从 arXiv 抓 PDF + 元数据，可选立刻调 MinerU 或 PyMuPDF 解析为 Markdown。
...
```bash
qatlas ingest 2501.00010 --parser pymupdf
```

执行流程：

1. client POST `/api/ingest/paper` 提交任务
2. server 异步从 arXiv 下载 PDF + 元数据 JSON 到 RawDir/S3
3. 用指定 parser 解析 PDF 为 Markdown
...
=== "只拿 PDF，不解析"
    ...
    ```bash
    qatlas ingest quant-ph/9508027 --parser pymupdf --stop-after fetch
    ```
...
=== "PyMuPDF 一步到位（无第三方调用）"
    ...
    PyMuPDF 是本地解析，速度快、纯文本结果中等。**不消耗任何外部 API 配额**。适合大量论文先做粗解析。

=== "MinerU 高质量解析"

    ```bash
    qatlas ingest 2501.00010 --parser mineru
    ```

    MinerU 是服务化的高质量解析（支持公式、表格、OCR），结果接近排印 Markdown 质量。**server 端要配 `MINERU_API_TOKEN`** —— 如果你的 server 没配，server 会回 400 并提示。这种情况下改用[本地 MinerU 模式](parse-with-mineru.md)。

## 续跑已存在的任务

如果一个 task 中途断了或者你想用不同 parser 重做：

```bash
qatlas ingest continue <task_id> --parser mineru --stages parse
```
...
!!! failure "503 MinerU not configured on server"
    Server 没配 `MINERU_API_TOKEN`。用 `--parser pymupdf` 或本地 MinerU 模式。
