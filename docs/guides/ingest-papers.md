...
!!! info "MinerU 是当前唯一受支持的解析器"
    早期版本曾包含一个本地 PDF 解析器作为离线选项，但因第三方 license 约束已从开源版本移除。所有摄入流程现在统一走 MinerU 远程 API（**需要服务端配置 `MINERU_API_TOKEN`**），或者跳过 server 解析 + 手动 `qatlas upload markdown` 推上去。
...
!!! failure "503 MinerU not configured on server"
    Server 没配 `MINERU_API_TOKEN`。请改走[本地 MinerU 模式](parse-with-mineru.md)（自己出 MinerU 配额，解析完再用 `qatlas upload markdown` 推回），或者联系 server 管理员补 token。
