...
应用仓库内的 `wiki/` 只作为本地测试/临时目录，不作为主仓库内容追踪。开发与生产推荐的最小覆盖只有 `QATLAS_WIKI_DIR`：

```env
QATLAS_WIKI_DIR=../QuantumAtlas-Wiki
# QATLAS_RAW_DIR、QATLAS_DATA_DIR 默认 raw/、data/，通常保留在仓库内即可；
# 只有在需要把论文资产搬到大容量盘等场景才显式覆盖。
```

> 旧名 `WIKI_DIR` / `RAW_DIR` / `DATA_DIR` 仍作 alias 兼容，新写法推荐 `QATLAS_*` 前缀。

`RAW_DIR` 和 `DATA_DIR` 留在仓库内能避免多一份路径同步的负担；它们都会被 `.gitignore` 排除，不会污染 Git 历史。
