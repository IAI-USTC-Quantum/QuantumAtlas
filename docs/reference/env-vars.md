...
> 服务端静默转换时 MinerU 主动拉取 PDF 的 presigned 直链，由各 edge 自己的 `QATLAS_S3_PUBLIC_ENDPOINT` 签发（每 edge 自包含，无跨 edge 依赖）。该公网入口必须被 MinerU 爬虫可达：RackNerd 走 `https://raw.quantum-atlas.ai`（LE 证书），Alibaba 走 `http://<ip>:9000`（纯 HTTP——未备案无法挂域名，自签 TLS 被 MinerU 拒）。
