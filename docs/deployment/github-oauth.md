...
| 字段 | 填什么 |
|---|---|
| **Application name** | `QuantumAtlas (RackNerd)` 之类（多边缘时按线路区分）|
| **Homepage URL** | `https://quantum-atlas.ai` |
| **Authorization callback URL** | `https://quantum-atlas.ai/api/oauth2-redirect` |
...
curl https://quantum-atlas.ai/api/collections/users/auth-methods | jq
# 应该看到 .oauth2.providers[] 里有 "github" 条目
```

或浏览器打开 `https://quantum-atlas.ai/` → 应该有 "Sign in with GitHub" 按钮。
...
**GitHub 限制每个 OAuth App 只允许一个 callback URL**，所以两台边缘部署时**必须建两个 OAuth App**：

| 边缘 | OAuth App name | Callback URL | client_id 写在哪 |
|---|---|---|---|
| RackNerd | QuantumAtlas (RackNerd) | `https://quantum-atlas.ai/api/oauth2-redirect` | RackNerd 的 `.env` |
| 阿里云 | QuantumAtlas (Alibaba) | `https://47.102.36.175/api/oauth2-redirect` | 阿里云的 `.env` |

各边缘启动时只注入自己那一份 OAuth provider，互不冲突。**同一 GitHub 账号在两边各登一次会建两条 users 记录**（不同 user id），PAT 也是独立的——参见 [多边缘部署](../concepts/multi-edge.md)。
