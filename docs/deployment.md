...
> **binary 改名后的 first deploy 注意**：本仓库 2026-05-28 把 binary 从
> `quantumatlas` 改名为 `qatlas-server`，systemd unit 名也从 `qatlas` 改成
> `qatlas-server`。第一次部署新名 binary 时要 disable 老 unit 再 install
> 新 unit：
>
> ```bash
> systemctl --user disable --now qatlas.service     # 老 unit；system-mode 加 sudo
> qatlas-server service install --mode user --force # 新 unit
> ```
>
> 一次到位。之后日常 deploy 回到上面 A / B 流程。
