...
用户级（per-user systemd 或 `--user` ExecStart，推荐这个）：
...
`$HOME` 自动算出默认。**只**在需要存到挂载盘 / 共享路径时显式覆盖
`QATLAS_RAW_DIR` 等。
...
# Hardening：read-only 系统目录 + 只把 .env / pb_data / wiki / 可选挂载点
# 打开写权限。注意 ReadWritePaths 必须覆盖到 cfg 里所有 stateful 路径
# （RAW_DIR / DATA_DIR / PBDataDir / WikiDir），按你的实际 .env 配置调整。
NoNewPrivileges=true
PrivateTmp=true
ProtectSystem=full
ProtectHome=no
ReadWritePaths=/home/<USER>/QuantumAtlas /home/<USER>/.local/share/quantum-atlas /home/<USER>/QuantumAtlas-Wiki /mnt/team
...
- 1810 prod 怎么从 `/mnt/team/QuantumAtlas/{raw,data}` 平滑过渡
...
时按 `$XDG_DATA_HOME` / `$HOME` 自动算出来。只有当默认值不合适
（比如想存到团队挂载盘）才在 `.env` 里覆盖。
...
# QATLAS_RAW_DIR=/mnt/team/QuantumAtlas/raw           # 默认 XDG，挂载盘覆盖
# QATLAS_DATA_DIR=/mnt/team/QuantumAtlas/data
# QATLAS_PB_DATA_DIR=/var/lib/quantum-atlas/pb_data   # 默认 XDG，FHS 覆盖
```
...
- 运行 QuantumAtlas 的服务用户应对 `RAW_DIR` / `DATA_DIR` / `PB_DATA_DIR` 有写权限。三者默认都落在 `$XDG_DATA_HOME/quantum-atlas/`（即 `$HOME/.local/share/quantum-atlas/`）下，正常的 systemd `User=<svc>` 已经自动满足；只在显式覆盖到挂载盘 / `/var/lib/...` 时检查权限。
...
完整内置默认值以 Go server 的 `internal/config/config.go` 为准（旧 Python
server 的 `atlas/server/config.py` 仍可参考，但已不是生产路径）。
`.env.example` 只是覆盖模板，**留空即用代码默认**，不应直接当成生产默认
配置。公网部署通常最关心下面这些：

| 变量 | 说明 | 默认 |
|------|------|------|
| `QATLAS_WIKI_DIR` | Wiki 知识库目录（alias: `WIKI_DIR`） | `<.env 目录>/../QuantumAtlas-Wiki`（兄弟 checkout） |
| `QATLAS_RAW_DIR` | canonical 论文资产根目录（alias: `RAW_DIR`） | `${XDG_DATA_HOME:-$HOME/.local/share}/quantum-atlas/raw` |
| `QATLAS_DATA_DIR` | 任务、share、ingest 状态目录（alias: `DATA_DIR`） | `${XDG_DATA_HOME:-$HOME/.local/share}/quantum-atlas/data` |
| `QATLAS_PB_DATA_DIR` | PocketBase pb_data（SQLite + uploads）（alias: `PB_DATA_DIR`） | `${XDG_DATA_HOME:-$HOME/.local/share}/quantum-atlas/pb_data` |
| `QATLAS_SERVER_URL` | 对外唯一根地址，client、share 链接和 MinerU URL 都基于它（alias: `PUBLIC_BASE_URL`） | — |
| `QATLAS_INSECURE` | client 跳过 TLS 校验的开关；等价于客户端 CLI 加 `--insecure` | `false` |
| `QATLAS_SHARE_ACCESS_TOKEN` | 可选的常驻 share token（alias: `SHARE_ACCESS_TOKEN`） | 空（不启用） |
| `QATLAS_USER_HEADER` | 可选的上游用户头；留空时 QuantumAtlas 不读取用户头（alias: `USER_HEADER`） | 空 |
| `QATLAS_SERVER_HOST` / `QATLAS_SERVER_PORT` | 监听地址和端口（alias: `SERVER_HOST` / `SERVER_PORT`） | `127.0.0.1:4200` |
| `NEO4J_URI` / `NEO4J_USER` / `NEO4J_PASSWORD` | 图数据库连接配置（Neo4j 官方驱动标准名） | — |
| `MINERU_*` | 使用 MinerU 解析时的可选配置（MinerU 厂家标准名） | — |
