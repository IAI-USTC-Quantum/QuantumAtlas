...
class SourceContractTests(unittest.TestCase):
    ...
    def test_reusable_ci_checks_go_formatting_and_both_docs_systems(self):
        ...
        self.assertIn("version: '0.11.30'", workflow)
        self.assertNotIn("actions/setup-python@v5", workflow)
        self.assertNotIn("python -m mkdocs", workflow)
        self.assertNotIn("cache-dependency-path: docs/requirements.txt", workflow)
        self.assertNotIn("cache-dependency-path: docsite/requirements.txt", workflow)
        self.assertIn("DOCS_READ_TOKEN", workflow)
        self.assertEqual(workflow.count("uv run --locked --script .github/scripts/build_docs.py"), 3)
        self.assertIn("node .github/scripts/docs-browser.cjs web/public build/docs-browser", workflow)
    ...
    def test_docs_image_reuses_clean_sphinx_build_without_legacy_publish(self):
        ...
        self.assertIn("'.github/scripts/build_docs.py'", workflow)
        self.assertIn("'.github/scripts/build_docs.py.lock'", workflow)
        self.assertIn('SOURCE_DATE_EPOCH="$(git show -s --format=%ct HEAD)"', workflow)
        self.assertIn("export SOURCE_DATE_EPOCH\n", workflow)
        self.assertIn("uv run --locked --script .github/scripts/build_docs.py --release", workflow)
        self.assertNotIn("sphinx-build ", workflow)
        self.assertIn("cp -a web/public/doc web/public/devdoc docs-dist/", workflow)
        self.assertIn("DOCS_READ_TOKEN", workflow)
        builder = (root / ".github/scripts/build_docs.py").read_text()
        self.assertEqual(builder.count('"-m", "sphinx"'), 1)
        self.assertEqual(builder.count('"-W", "--keep-going", "-n", "-b", "html"'), 2)
